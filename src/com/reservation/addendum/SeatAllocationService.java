 ...
 */
	...
	public static ArrayList<Seat> getSeatsInARow(ArrayList<Integer> seatNumberList, int numSeats, int rowSize, String rowName,String levelName, String userEmail){
		...
		if(size == 0){
			for (int i = 1; i <= numSeats; i++){
				...
				Seat seat = encapsulateSeatInformation(userEmail, i, rowName, levelName, endTime,status);
		...
		for(Integer seatNumber : seatNumberArray){
			...
			endTime.add(Calendar.SECOND, DataLoader.ticketHoldTimeOut);
			String status = "Blocked";
			Seat seat = encapsulateSeatInformation(userEmail, seatNumber, rowName, levelName, endTime,status);
			freeSeatArray.add(seat);
		}
	 ...
	 * @return Seat
	 */
	public static Seat encapsulateSeatInformation(String userEmail, Integer seatNumber, String row, String level, Calendar endTime,String status){
		Seat seat = new Seat();
